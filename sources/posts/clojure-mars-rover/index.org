...
* Finally: "Imperative shell" to parse incoming signal
  ...
  must absorb the insanity of data inbound from the wild.

  Someone has to do it. Might as well be us. There are many ways to do it.
  Here is one way.
  ...
  #+begin_src clojure
    ...
    (comment
      (let [_ (spit "rover-commands.txt"
        ...
        (map calculate-path input-command-data))
      )
  #+end_src
* Recap
